...
class Handle_tags_ugv:
	def __init__(self, biggest):
		self.biggest = handleTags.biggest
	
	def up_ugv():
		print "following tag"
		biggest = Tag()
		if self.biggest.diameter == 0:
			twist = Twist()
			twist.linear.x = 0
			if (time() - lastSeen > .5):
...
class toggle_cam:
	def __init__(self):
		self.camselectclient = rospy.ServiceProxy('/ardrone/togglecam', CamSelect)
